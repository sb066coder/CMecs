...
//    )
    ...
    */
    fun move(forces: Array<Force>, t: Int = TIME_STEP) {
        val accX = forces.fold(0.0) { acc, force -> acc + force.xComp } / mass //{ acc, force -> acc + (force.xComp + force.xDum) / 2 } / mass
        val accY = forces.fold(0.0) { acc, force -> acc + force.yComp } / mass //{ acc, force -> acc + (force.yComp + force.yDum) / 2 } / mass
        val accZ = forces.fold(0.0) { acc, force -> acc + force.zComp } / mass //{ acc, force -> acc + (force.zComp + force.zDum) / 2 } / mass
        ...
        velocity.maxSpeed = maxOf(velocity.maxSpeed, velocity.getSpeed())
        velocity.minSpeed = minOf(velocity.minSpeed, velocity.getSpeed())
